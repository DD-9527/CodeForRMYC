...
void gamesir_task(void *arg)
{
    ...
    for (;;)
    {
        ...
        else if (p_tt_gamesir->DataIsValid())
        {
            ...
            int ly = ((float)data.left_y_3d - 512) / 512.0 * 130;
            int rx = ((float)data.right_x_3d - 512) / 512.0 * 80;
            int ry = ((float)data.right_y_3d - 512) / 512.0 * 80;
            ...
            else if ((data.X) && (data.R2))
            {
                
                ledcWrite(channel, calculatePWM(160));
            }
            else if ((data.B) && (data.R2))
            {
                ledcWrite(channel, calculatePWM(98));
            }
            else
            {
